...
const BackOfficeShiftsTable = ({
    ...
    const columns: ColumnDef<TBackOfficeShift>[] = useMemo(
        () => [
            {
                header: t('backOffice.shiftsTable.name'),
                accessorKey: 'name',
                cell: ({ row }) => (
            ...
            {
                header: t('backOffice.shiftsTable.type'),
                accessorKey: 'type',
                size: 70,
            },
            {
                header: t('backOffice.shiftsTable.distribution'),
                accessorKey: 'distribution',
            },
            {
                header: t('backOffice.shiftsTable.data'),
                accessorKey: 'data',
                cell: ({ row }) => (
                    <div className="flex items-center h-10">
                        <p className="line-clamp-2">
                            <b>{row.original.data.hoursPerDay}</b> al día <br />
                            <b>{row.original.data.weeklyHours}</b> horas a la
                            semana
                        </p>
                    </div>
                ),
            },
            {
                header: (
                    ...
                    | ColumnDefTemplate<
                          HeaderContext<TBackOfficeShift, unknown>
                      >
                    | undefined,
                size: 60,
                id: 'actions',
        ...
        [t, editMode, setTempShift],
    )

    const renderTable = useMemo(
        () => (
            <DataTable<TBackOfficeShift>
                arrowPagination
    ...
    return (
        <div id={id}>
            <h4 className="heading-text text-xl font-bold mb-2">
                {t('backOffice.shiftsTable.title')}
            </h4>
            <p className="text-gray-500 mb-4">
                {t('backOffice.shiftsTable.description')}
            </p>
            <AdaptiveCard
                footer={{
                    className:
...
export default BackOfficeShiftsTable
