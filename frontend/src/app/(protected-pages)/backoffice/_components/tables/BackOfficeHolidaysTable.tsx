...
import { Button, Input, Dialog, InputGroup, Tag } from '@/components/ui'
...
const BackOfficeHolidaysTable = ({
    ...
    const filteredData = useMemo(() => {
        ...
        return data.filter((holiday) =>
            holiday.date?.toLowerCase().includes(searchValue.toLowerCase()),
        )
    }, [data, searchValue])
    ...
    const columns: ColumnDef<TBackOfficeHoliday>[] = useMemo(
        () => [
            {
                header: t('backOffice.holidaysTable.date'),
                accessorKey: 'date',
                cell: ({ row }) => (
            ...
            },
            {
                ...
                cell: ({ row }) => (
                    <Tag>
                        {row.original.type
                            ? getHolidayTypeText(row.original.type, t)
                            : '--'}
                    </Tag>
                ),
            },
            {
                header: (
                    ...
                    | ColumnDefTemplate<
                          HeaderContext<TBackOfficeHoliday, unknown>
                      >
                    | undefined,
                id: 'actions',
                size: 60,
        ...
        [t, editMode, setTempHoliday],
    )

    const renderTable = useMemo(
        () => (
            <DataTable<TBackOfficeHoliday>
                arrowPagination
    ...
    return (
        <div id={id}>
            <h4 className="heading-text text-xl font-bold mb-2">
                {t('backOffice.holidaysTable.title')}
            </h4>
            <p className="text-gray-500 mb-4">
                {t('backOffice.holidaysTable.description')}
            </p>
            <AdaptiveCard
                footer={{
                    className:
...
export default BackOfficeHolidaysTable
