...
const BackOfficeChoresTable = ({
    ...
    const filteredData = useMemo(() => {
        ...
        return data.filter(
            (chore) =>
                chore.name?.toLowerCase().includes(searchValue.toLowerCase()) ||
                chore.code?.toLowerCase().includes(searchValue.toLowerCase()),
        )
    }, [data, searchValue])
    ...
    const columns: ColumnDef<TBackOfficeChore>[] = useMemo(
        () => [
            {
                header: t('backOffice.choresTable.name'),
                accessorKey: 'name',
                cell: ({ row }) => (
            ...
            {
                header: t('backOffice.choresTable.code'),
                accessorKey: 'code',
                cell: ({ row }) => (
                    <p className="line-clamp-2 text-ellipsis overflow-hidden">
                        {row.original.code || '--'}
            ...
            },
            {
                header: (
                    ...
                    | ColumnDefTemplate<
                          HeaderContext<TBackOfficeChore, unknown>
                      >
                    | undefined,
                id: 'actions',
                size: 60,
        ...
        [t, editMode, setTempChore],
    )

    const renderTable = useMemo(
        () => (
            <DataTable<TBackOfficeChore>
                arrowPagination
    ...
    return (
        <div id={id}>
            <h4 className="heading-text text-xl font-bold mb-2">
                {t('backOffice.choresTable.title')}
            </h4>
            <p className="text-gray-500 mb-4">
                {t('backOffice.choresTable.description')}
            </p>
            <AdaptiveCard
                footer={{
                    className:
...
export default BackOfficeChoresTable
