...
const BackOfficeCertificatesTable = ({
    ...
    const filteredData = useMemo(() => {
        ...
        return data.filter(
            (cert) =>
                ...
                cert.code?.toLowerCase().includes(searchValue.toLowerCase()),
        )
    }, [data, searchValue])
    ...
    const columns: ColumnDef<TBackOfficeCertificate>[] = useMemo(
        () => [
            {
                header: t('backOffice.certificatesTable.name'),
                accessorKey: 'name',
                cell: ({ row }) => (
            ...
            {
                header: t('backOffice.certificatesTable.code'),
                accessorKey: 'code',
            },
            {
                header: t('backOffice.certificatesTable.type'),
                accessorKey: 'type',
            },
            {
                header: t('backOffice.certificatesTable.relevance'),
                accessorKey: 'relevance',
                cell: ({ row }) => (
                    <Tooltip
                        title={
                            getRelevanceText(row.original.relevance, t) || '--'
                        }
                    >
            ...
            {
                header: t('backOffice.certificatesTable.matrix'),
                accessorKey: 'matrix',
                cell: ({ row }) =>
                    row.original.matrix
                        ? getMatrixText(row.original.matrix, t)
                        : '--',
            },
            {
                header: (
                    ...
                    | ColumnDefTemplate<
                          HeaderContext<TBackOfficeCertificate, unknown>
                      >
                    | undefined,
                size: 60,
                id: 'actions',
    ...
    const renderTable = useMemo(
        () => (
            <DataTable<TBackOfficeCertificate>
                arrowPagination
    ...
    return (
        <div id={id}>
            <h4 className="heading-text text-xl font-bold mb-2">
                {t('backOffice.certificatesTable.title')}
            </h4>
            <p className="text-gray-500 mb-4">
                {t('backOffice.certificatesTable.description')}
            </p>
            <AdaptiveCard
                footer={{
                    className:
            ...
            <BackOfficeCreateCertificateModal
                isOpen={modalOpen}
...
export default BackOfficeCertificatesTable
