...
from models.ddb.Certificate import CertificatePayload
from models.General import PatchCertificateBodyRequest
...
def get():
    try:
        ...
        return error_response("GetCertificateError", str(e))


@router.post("/")
def post():
    try:
        body = CertificatePayload(**router.current_event.json_body)
        certificate = body.model_dump(exclude_none=True)
        hash = generate_unique_hash()
        _ = put_item(pk, f"CERTS#{hash}", certificate)
...
def patch():
    try:
        body = PatchCertificateBodyRequest(**router.current_event.json_body)
        body.updateAt = datetime.now().isoformat()
        certificate = update_item(
